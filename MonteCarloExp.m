...
for pn = 0:poseNum
    ...
    %% ideal EKF
    DeltaR1XpIdeFullSet((end+1):(end+3),:) = R1XpIdeFullSet(pn*3+(1:3),2:end) - [R1XrTrue(pn*2+(1:2),2);R1XphiT(pn+1,2)];
    DeltaR2XpIdeFullSet((end+1):(end+3),:) = R2XpIdeFullSet(pn*3+(1:3),2:end) - [R2XrTrue(pn*2+(1:2),2);R2XphiT(pn+1,2)];

    % wrap the delta angle
    DeltaR1XpIdeFullSet(end,:) = wrap(DeltaR1XpIdeFullSet(end,:));
    DeltaR2XpIdeFullSet(end,:) = wrap(DeltaR2XpIdeFullSet(end,:));
...
% standard EKF
DeltaFFullSet = [XfFullSet(:,1),XfFullSet(:,2:end)-XfTrue(:,2)];
% ideal EKF
DeltaFIdeFullSet = XfIdeFullSet(:,2:end)-XfTrue(:,2);

%% save the Monte Carlo Experiments result
save('MTE_results_StdEKF.mat','poseNum','feaNum', ...
