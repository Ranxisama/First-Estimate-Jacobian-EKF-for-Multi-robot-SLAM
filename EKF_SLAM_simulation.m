...
for k = 0:size(R1Odo,1)/3
    ...
    if k == 0
        ...
        if step0Check == 1
            ...
            hold on
            % true state
            R1trueP = plot(R1XrTrue(1:2:(end-1),2),R1XrTrue(2:2:end,2),'-bo','DisplayName','R1 True pose');
            R2trueP = plot(R2XrTrue(1:2:(end-1),2),R2XrTrue(2:2:end,2),'-co','DisplayName','R2 True pose');
            FeaTrueP = plot(XfTrueAll(1:2:(end-1),2),XfTrueAll(2:2:end,2),'*','Color',oliveGreen,'DisplayName','True features');
            % estimated state
            R1Xr0P = plot(X0(1,3),X0(2,3),'ro','DisplayName','R1 estimated pose');
            R2Xr0P = plot(X0(4,3),X0(5,3),'mo','DisplayName','R2 estimated pose');
            Xf0P = plot(X0(7:2:(end-1),3),X0(8:2:end,3),'*','Color',lightGreen,'DisplayName','Estimated features of GNI');
            text(X0(7:2:(end-1),3),X0(8:2:end,3),num2str(X0(7:2:(end-1),2)),'Color',lightGreen);
            ...
            % R1
            % eeR1Xr0e = errorEllipse(P0(1:2,1:2), X0(1:2,3), CI);
            % eeR1Xr0eP = plot(eeR1Xr0e(:,1), eeR1Xr0e(:,2),'Color','r','DisplayName','Estimated R1 pose Cov ellipse');
            % R2
            eeR2Xr0e = errorEllipse(P0(4:5,4:5), X0(4:5,3), CI);
            eeR2Xr0eP = plot(eeR2Xr0e(:,1), eeR2Xr0e(:,2),'Color','m','DisplayName','Estimated R2 pose Cov ellipse');

            eeXf0PHandles = gobjects(1, (size(X0,1)-6)/2);
            for j = 1:((size(X0,1)-6)/2)
                eeXf0 = errorEllipse(P0(6+(j-1)*2+(1:2),6+(j-1)*2+(1:2)), X0(6+(j-1)*2+(1:2),3), CI);
                eeXf0PHandles(j) = plot(eeXf0(:,1),eeXf0(:,2),'Color',lightGreen);
            end
            eeXf0PHandles(1).DisplayName = 'Estimated features Cov ellipse of GNI';
        ...
        if step0Check == 1
            ...
            hold on
            % estimated state
            Xf00eP = plot(Xk00e((size(X0,1)+1):2:(end-1),3),Xk00e((size(X0,1)+2):2:end,3),'*','Color',cyanGreen,'DisplayName','Estimated features of FI');
            text(Xk00e((size(X0,1)+1):2:(end-1),3),Xk00e((size(X0,1)+2):2:end,3),num2str(Xk00e((size(X0,1)+1):2:(end-1),2)),'Color',cyanGreen);


            % Plot the ellipse of initialized features
            eeXf00ePHandles = gobjects(1,(size(Xk00e,1)-size(X0,1))/2);
            for j = 1:((size(Xk00e,1)-size(X0,1))/2)
                ...
                eeXf00ePHandles(j) = plot(eeXfk00e(:,1),eeXfk00e(:,2),'Color',cyanGreen);
            end
            eeXf00ePHandles(1).DisplayName = 'Estimated features Cov ellipse of FI';
             legend([R1trueP, R2trueP, FeaTrueP, ...
                 R1Xr0P, R2Xr0P, Xf0P, ...
                 eeR2Xr0eP, eeXf0PHandles(1), ...
                 Xf00eP, eeXf00ePHandles(1)]);
    ...
    if predictCheck == 1
        ...
        FeaTrueP = plot(XfTrueAll(1:2:(end-1),2),XfTrueAll(2:2:end,2),'*','Color',oliveGreen,'DisplayName','True features');
        % estimated state
        R1Xk10eP = plot(Xk10e(1,3),Xk10e(2,3),'-ro','DisplayName','R1 estimated pose after prediction');
        R2Xk10eP = plot(Xk10e(4,3),Xk10e(5,3),'-mo','DisplayName','R2 estimated pose after prediction');
        % Xpk = [R1XrTrue(R1XrTrue(:,1)==k,:);R1XphiT(R1XphiT(:,1)==k,:);R2XrTrue(R2XrTrue(:,1)==k,:);R2XphiT(R2XphiT(:,1)==k,:)]

        % R1
        eeR1Xr10e = errorEllipse(Pk10(1:2,1:2), Xk10e(1:2,3), CI);
        eeR1Xr10eP = plot(eeXr10e(:,1), eeR1Xr10e(:,2),'Color','r','DisplayName','R1 estimated pose Cov ellipse after prediction');
        % R2
        eeR2Xr10e = errorEllipse(Pk10(4:5,4:5), Xk10e(4:5,3), CI);
        eeR2Xr10eP = plot(eeR2Xr10e(:,1), eeR2Xr10e(:,2),'Color','m','DisplayName','R2 estimated pose Cov ellipse after prediction');

        legend([R1trueP,R2trueP,FeaTrueP, ...
            R1Xk10eP,R2Xk10eP, ...
            eeR1Xr10eP,eeR2Xr10eP])
    ...
    if fiCheck == 1 && ~isempty(Xfn)
        figure(2)
        hold on
        XfnP = plot(Xfn(1:2:(end-1),2),Xfn(2:2:end,2),'*','Color',cyanGreen,'DisplayName','R1 estimated pose of FI');
        % text(Xfn(1:2:(end-1),2),Xfn(2:2:end,2),num2str(Xfn(1:2:(end-1),1)),'Color',mediumLightGreen)


        % new features
        eeXf10ePHandle = gobjects(1,size(Xfn,1)/2);
        for j = 1:(size(Xfn,1)/2)
            eeXf10e = errorEllipse(Pk10fi(size(Pk10,1)+(j-1)*2+(1:2),size(Pk10,1)+(j-1)*2+(1:2)), Xfn((j-1)*2+(1:2),2), CI);
            eeXf10ePHandle(j) = plot(eeXf10e(:,1), eeXf10e(:,2),'Color',cyanGreen);
        end
        eeXf10ePHandle(1).DisplayName = 'R1 estimated pose Cov ellipse of FI';

        legend(legendStrings)
        title(sprintf('step %d with %d features of feature initialization',k,size(Xfn,1)/2))
