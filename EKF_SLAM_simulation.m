...
for k = 0:size(R1Odo,1)/3
    ...
    if k == 0
        ...
        % hold on
        % % true state
        % R1trueP1 = plot(R1XrTrue(1:2:(end-1),2),R1XrTrue(2:2:end,2),'-bo','DisplayName','R1 True pose');
        % R2trueP1 = plot(R2XrTrue(1:2:(end-1),2),R2XrTrue(2:2:end,2),'-co','DisplayName','R2 True pose');
        % FeaTrueP1 = plot(XfTrueAll(1:2:(end-1),2),XfTrueAll(2:2:end,2),'*','Color',darkGreen,'DisplayName','True features');
        % % estimated state
        % R1Xr0P = plot(X0(1,3),X0(2,3),'ro','DisplayName','R1 estimated pose');
        % R2Xr0P = plot(X0(4,3),X0(5,3),'mo','DisplayName','R2 estimated pose');
        % Xf0P = plot(X0(7:2:(end-1),3),X0(8:2:end,3),'*','Color','y','DisplayName','Estimated features of GNI');
        % % text(X0(7:2:(end-1),3),X0(8:2:end,3),num2str(X0(7:2:(end-1),2)),'Color','y');
        %
        %
        % hold off
        ...
        FeaTrueP1 = plot(XfTrueAll(1:2:(end-1),2),XfTrueAll(2:2:end,2),'*','Color',darkGreen,'DisplayName','True features');
        % estimated state
        R1Xrk00P = plot(Xk00e(1,3),Xk00e(2,3),'ro','DisplayName','R1 estimated pose');
        R2Xr0P = plot(R2Xp0(1,1),R2Xp0(2,1),'o','Color',[0.5, 0, 0.5],'DisplayName','R2 initial GNI pose');
        R2Xrk00P = plot(Xk00e(4,3),Xk00e(5,3),'mo','DisplayName','R2 estimated pose');
        Xfk00eP = plot(Xk00e(7:2:(end-1),3),Xk00e(8:2:end,3),'*','Color','y','DisplayName','Estimated features of GNI');
        % text(Xk00e((size(X0,1)+1):2:(end-1),3),Xk00e((size(X0,1)+2):2:end,3),num2str(Xk00e((size(X0,1)+1):2:(end-1),2)),'Color','y');

        if poseCovCheck == 1
            % eeR1Xrk00 = errorEllipse(Pk00(1:2,1:2), Xk00e(1:2,3), CI);
            eeR2Xr0 = errorEllipse(R2O(1:2,1:2),R2Xp0(1:2,1),CI);
            eeR2Xr0P = plot(eeR2Xr0(:,1),eeR2Xr0(:,2),'Color',[0.5, 0, 0.5],'DisplayName','R2 estimated pose Cov');
            eeR2Xrk00 = errorEllipse(Pk00(4:5,4:5), Xk00e(4:5,3), CI);
            % eeR1Xrk00P = plot(eeR1Xrk00(:,1),eeR1Xrk00(:,2),'Color','r','DisplayName','R1 estimated pose Cov');
            eeR2Xrk00P = plot(eeR2Xrk00(:,1),eeR2Xrk00(:,2),'Color','m','DisplayName','R2 initial GNI pose Cov');

            if feaCovCheck == 1
                % Plot the ellipse of initialized features
                eeXfk00ePHandles = gobjects(1,size(Xk00e,1)-6);
                for j = 1:((size(Xk00e,1)-6)/2)
                    eeXfk00e = errorEllipse(Pk00(6+(j-1)*2+(1:2),6+(j-1)*2+(1:2)), Xk00e(6+(j-1)*2+(1:2),3), CI);
                    eeXfk00ePHandles(j) = plot(eeXfk00e(:,1),eeXfk00e(:,2),'Color','y');
                end
                eeXfk00ePHandles(1).DisplayName = 'Estimated features Cov ellipse of GNI';
            end
        end
        hold off

        Xfk00eP.DisplayName = 'Estimated features of Gauss Newton Iteration';

        if feaCovCheck == 1
            Labels0 = [R1trueP1, R2trueP1, FeaTrueP1, ...
                R2Xr0P,R1Xrk00P, R2Xrk00P, Xfk00eP, ...
                eeR2Xr0P,eeR2Xrk00P,eeXfk00ePHandles(1)];
        else
            if poseCovCheck == 1
                Labels0 = [R1trueP1, R2trueP1, FeaTrueP1, ...
                    R2Xr0P,R1Xrk00P, R2Xrk00P, Xfk00eP, ...
    ...
    if predictCheck == 1
        ...
        if k == 1
            % true state
            R1trueP2 = plot(R1XrTrue(1:2:(end-1),2),R1XrTrue(2:2:end,2),'-bo','DisplayName','R1 True pose');
            R2trueP2 = plot(R2XrTrue(1:2:(end-1),2),R2XrTrue(2:2:end,2),'-co','DisplayName','R2 True pose');
            FeaTrueP2 = plot(XfTrueAll(1:2:(end-1),2),XfTrueAll(2:2:end,2),'*','Color',paleGreen,'DisplayName','True features');
        end
        % estimated state
        R1Xrk10eP = plot(Xk10e(1,3),Xk10e(2,3),'-ro');
        R2Xrk10eP = plot(Xk10e(4,3),Xk10e(5,3),'-mo');
        % Xpk = [R1XrTrue(R1XrTrue(:,1)==k,:);R1XphiT(R1XphiT(:,1)==k,:);R2XrTrue(R2XrTrue(:,1)==k,:);R2XphiT(R2XphiT(:,1)==k,:)]

        if poseCovCheck == 1
            % R1
            eeR1Xrk10e = errorEllipse(Pk10(1:2,1:2), Xk10e(1:2,3), CI);
            eeR1Xrk10eP = plot(eeR1Xrk10e(:,1), eeR1Xrk10e(:,2),'Color','r');
            % R2
            eeR2Xrk10e = errorEllipse(Pk10(4:5,4:5), Xk10e(4:5,3), CI);
            eeR2Xrk10eP = plot(eeR2Xrk10e(:,1), eeR2Xrk10e(:,2),'Color','m');
        end
        hold off

        if k == 1
            R1Xrk10eP.DisplayName = 'Estimated R1 pose after prediction';
            R2Xrk10eP.DisplayName = 'Estimated R2 pose after prediction';
            if poseCovCheck == 1
                eeR1Xrk10eP.DisplayName = 'Estimated R1 pose Cov ellipse after prediction';
                eeR2Xrk10eP.DisplayName = 'Estimated R2 pose Cov ellipse after prediction';
            end
            if feaCovCheck == 1
                Labelspred = [R1trueP2,R2trueP2,FeaTrueP2, ...
                    R1Xrk10eP,R2Xrk10eP,Xfk00eP, ...
                    eeR1Xrk10eP,eeR2Xrk10eP, eeXfk00ePHandles(1)];
            else
                if poseCovCheck == 1
                    Labelspred = [R1trueP2,R2trueP2,FeaTrueP2, ...
                        R1Xrk10eP,R2Xrk10eP,Xfk00eP, ...
                        eeR1Xrk10eP,eeR2Xrk10eP];
                else
                    Labelspred = [R1trueP2,R2trueP2,FeaTrueP2, ...
                        R1Xrk10eP,R2Xrk10eP,Xfk00eP];
                end
        ...
        title(sprintf('step %d with %d new features after prediction',k,(size(Xk10e,1)-6)/2))
    ...
    if fiCheck == 1 && ~isempty(Xfn)
        figure(2)
        hold on
        XfknP = plot(Xfn(1:2:(end-1),2),Xfn(2:2:end,2),'*','Color',paleGreen,'DisplayName','R1 estimated pose of FI');
        % text(Xfn(1:2:(end-1),2),Xfn(2:2:end,2),num2str(Xfn(1:2:(end-1),1)),'Color',medium'y')

        if feaCovCheck == 1
            % new features
            eeXfk10ePHandle = gobjects(1,size(Xfn,1)/2);
            for j = 1:(size(Xfn,1)/2)
                eeXfk10e = errorEllipse(Pk10fi(size(Pk10,1)+(j-1)*2+(1:2),size(Pk10,1)+(j-1)*2+(1:2)), Xfn((j-1)*2+(1:2),2), CI);
                eeXfk10ePHandle(j) = plot(eeXfk10e(:,1), eeXfk10e(:,2),'Color',paleGreen);
            end
            eeXfk10ePHandle(1).DisplayName = 'R1 estimated pose Cov ellipse of FI';
        end
        hold off
        if k == 1
            if feaCovCheck == 1
                Labelsfi = [R1trueP2,R2trueP2,FeaTrueP2, ...
                    R1Xrk10eP,R2Xrk10eP,XfknP, ...
                    eeR1Xrk10eP,eeR2Xrk10eP, eeXfk10ePHandle(1)];
            else
                if poseCovCheck == 1
                    Labelsfi = [R1trueP2,R2trueP2,FeaTrueP2, ...
                        R1Xrk10eP,R2Xrk10eP,XfknP, ...
        ...
        title(sprintf('step %d with %d new features after feature initialization',k,size(Xfn,1)/2))
    ...
    figure(1)
    hold on
    R1Xr11eP = plot(Xk11e(1,3),Xk11e(2,3),'ro');
    R2Xr11eP = plot(Xk11e(4,3),Xk11e(5,3),'mo');

    if updateCheck == 1
        if poseCovCheck == 1
            eeR1Xr11e = errorEllipse(Pk11(1:2,1:2), Xk11e(1:2,3), CI);
            eeR2Xr11e = errorEllipse(Pk11(4:5,4:5), Xk11e(4:5,3), CI);

            eeR1Xr11eP = plot(eeR1Xr11e(:,1),eeR1Xr11e(:,2),'Color','r');
            eeR2Xr11eP = plot(eeR2Xr11e(:,1),eeR2Xr11e(:,2),'Color','m');
        end
        ...
        if k == 1
            R1Xr11eP.DisplayName = 'Estimated R1 pose after update';
            R2Xr11eP.DisplayName = 'Estimated R2 pose after update';
            Xf11eP.DisplayName = 'Estimated features after update';

            if poseCovCheck == 1
                eeR1Xr11eP.DisplayName = 'Estimated R1 pose Cov ellipse after update';
                eeR2Xr11eP.DisplayName = 'Estimated R2 pose Cov ellipse after update';

                if feaCovCheck == 1
                    eeXf11ePHandles(1).DisplayName = 'Estimated R2 pose Cov ellipse after update';
                end
            end

            if feaCovCheck == 1
                Labelsupdate = [R1trueP1, R2trueP1, FeaTrueP1, ...
                    R1Xrk00P, R2Xrk00P, Xfk00eP,Xf11eP, ...
                    eeR1Xr11eP,eeR2Xr11eP,eeXfk00ePHandles(1),eeXf11ePHandles(1)];
            else
                if poseCovCheck == 1
                    Labelsupdate = [R1trueP1, R2trueP1, FeaTrueP1, ...
                        R1Xrk00P, R2Xrk00P, Xfk00eP,Xf11eP, ...
                        eeR1Xr11eP,eeR2Xr11eP];
                else
                    Labelsupdate = [R1trueP1, R2trueP1, FeaTrueP1, ...
                        R1Xrk00P, R2Xrk00P, Xfk00eP,Xf11eP];
                end
...
plot(R1XpFull(1:3:(end-2),2),R1XpFull(2:3:(end-1),2),'--ro');
plot(R2XpFull(1:3:(end-2),2),R2XpFull(2:3:(end-1),2),'--mo');
...
xlabel('x')
ylabel('y')
title('Robot pose of truth vs multi-robot EKF')
